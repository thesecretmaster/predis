...
int command_ht_store(struct command_ht *ht, const char *command_name, command_func command, union command_preload_strategies preload, bool preload_func) {
...
command_func command_ht_fetch_command(struct command_ht *ht, char *command_name) {
  ...
  unsigned int index = base_index;
  while (ht->elements[index].command != NULL && strcmp(command_name, ht->elements[index].command_name) != 0) {
    index = index + 1 % ht->size;
    if (index == base_index)
      return NULL;
  }
...
union command_preload_strategies command_ht_fetch_preload(struct command_ht *ht, char *command_name, bool *preload_strat) {
  unsigned int base_index = hash((unsigned char*)command_name) % ht->size;
  unsigned int index = base_index;
  while (ht->elements[index].command != NULL && strcmp(command_name, ht->elements[index].command_name) != 0) {
    index = index + 1 % ht->size;
